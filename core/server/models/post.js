...
Post = ghostBookshelf.Model.extend({
  ...
  */
  ...
  //30
  onSaving: function onSaving(model, attr, options2) {
    var coverage = new Array(30).fill(false);
    var options={};
    if(options2){
      //console.log("b01");
      coverage[0]=true;
    ...
    if (newStatus === 'scheduled') {
      ...
      if (publishedAtHasChanged){
        //console.log("b08");
        coverage[7]=true;
        if(moment(publishedAt).isBefore(moment().add(config.get('times').cannotScheduleAPostBeforeInMinutes, 'minutes'))){
          //console.log("b09");
          coverage[8]=true;
    ...
    if (!_.isUndefined(this.get('tags'))) {
      ...
      if(!_.isNull(this.get('tags'))){
        ...
        _.each(this.get('tags'), function each(item) {
          for (i = 0; i < tagsToSave.length; i = i + 1) {
            if (tagsToSave[i].name &&
              item.name &&
              (tagsToSave[i].name.toLocaleLowerCase() === item.name.toLocaleLowerCase())) {
      ...
      var bool = this.hasChanged('html');
      if(!bool){
        //console.log("b14");
        coverage[13]=true;
        bool = !this.get('plaintext');
      }
      if (bool) {
        //console.log("b15");
        coverage[14]=true;
      ...
      if (!options.importing) {
        ...
        if(!title){
          //console.log("b17");
          coverage[16]=true;
          title = common.i18n.t('errors.models.post.untitled');
        }
      ...
      var boo1,boo2=false;
      boo1=(newStatus === 'published');
      if(boo1){
        //console.log("b20");
        coverage[19]=true;
        boo2 = this.hasChanged('status');
      }
      if (boo2) {
        //console.log("b21");
        coverage[20]=true;
        // unless published_by is set and we're importing, set published_by to contextUser
        bool =!this.get('published_by');
        if(!bool){
          //console.log("b22");
          coverage[21]=true;
          bool = ! options.importing;
        }
        if (bool) {
          //console.log("b23");
          coverage[22]=true;
        ...
        if (this.hasChanged('published_by') ) {
          //console.log("b24");
          coverage[23]=true;
          if(!options.importing){
            //console.log("b25");
      ...
      bool1 = (prevTitle !== undefined);
      if (bool1) {
        //console.log("b26");
        coverage[25]=true;
        bool2 = (newTitle !== prevTitle);
      }
      if (bool2) {
        //console.log("b27");
        coverage[26]=true;
        bool3 = (newStatus === 'draft');
      }
      if (bool3) {
        //console.log("b28");
        coverage[27]=true;
